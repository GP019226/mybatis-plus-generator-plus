...
import com.serendipity.constant.DataBaseConstant;
import com.serendipity.enums.DataTypeEnum;
import com.serendipity.pojo.vo.EntityAttrVO;
import com.serendipity.pojo.bo.Config;
import com.serendipity.pojo.bo.DbColumn;
...
public class InitDb {
    ...
    public List<EntityVO> initTables() {
        ...
        if (DataBaseConstant.DATABASE_ORACLE.equals(config.getJdbcType())) {
            ...
            sql.append("SELECT table_name,table_comment FROM information_schema.TABLES WHERE 1=1");
            sql.append(" and table_schema = '").append(config.getDataBaseName()).append("'");
            sql.append(" and table_name IN(").append(tableNamesJoin).append(")");
        } else {
    ...
    private List<DbColumn> initDbVo() {
        ...
        if (DataBaseConstant.DATABASE_ORACLE.equals(config.getJdbcType())) {
            ...
            sb.append("SELECT table_name,column_name,data_type,column_comment,column_key FROM information_schema.COLUMNS WHERE 1=1");
            if (StringUtils.isNotEmpty(tableNamesJoin)) {
                sb.append(" and table_schema = '").append(config.getDataBaseName()).append("'");
                sb.append(" and table_name in (").append(tableNamesJoin).append(")");
            }
